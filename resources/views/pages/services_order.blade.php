            ...
            <table class="table table-hover my-0">
                ...
                <tbody>
                    @foreach ($services as $service)
                        <tr>
                            <td class="text-center">{{ $service->product->name }}</td>
                            <td class="text-center"> {{ $service->customer }} </td>
                            <td class="text-center d-xl-table-cell">{{ $service->pickup_phone }}</td>
                            ...
                            <td class="d-md-table-cell">
                                <div class="gap-1 d-flex align-center justify-content-center">
                                    @if ($service->status != 3 && $service->status == 0)
                            ...
                            <div class="col-12 col-md-6">
                                <div style="font-size: 16px;">
                                    ...
                                    <input type="text" name="customer" class="form-control my-1"
                                        placeholder="Tên khách hàng" value="{{ old('customer', @$service->customer) }}">
                                </div>
                            ...
                            <div class="col-12 col-md-6">
                                <div style="font-size: 16px;"><strong>Số điện thoại:</strong>
                                    <input type="text" name="pickup_phone"
                                        value="{{ old('pickup_phone', @$service->pickup_phone) }}" class="form-control my-1"
                                        placeholder="Số điện thoại khách hàng">
                                </div>
                                <div style="font-size: 16px;">
                                    <strong>Thời gian đặt lịch:</strong>
                                    <input type="datetime-local" name="booking_time" class="form-control my-1" required
                                        min="{{ \Carbon\Carbon::now()->format('Y-m-d\T08:00') }}"
                                        max="{{ \Carbon\Carbon::now()->addDays(30)->format('Y-m-d\T20:00') }}"
                                        value="{{ optional($service->booking_time)->format('Y-m-d\TH:i') }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
@section('js')
    ...
    @endif
    <script src="{{ asset('frontend/script/services-order.js') }}"></script>
    <script>
