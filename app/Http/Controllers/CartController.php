...
class CartController extends Controller
{
    ...
    public function order(Request $request)
    {
        ...
        $validatedDataOrder['booking_date'] = $request->booking_date;
        ...
        // Chỉ xử lý giảm tồn kho nếu sản phẩm không thuộc category 1 hoặc 4
        if (!in_array($productModel->id_category, [1, 4])) {
            if ($productModel->amount >= $product['quantity']) {
                $productModel->decrement('amount', $product['quantity']);
            } else {
                // Xử lý lỗi tồn kho không đủ
                return back()->withErrors([
                    'Sản phẩm "' . $productModel->name . '" không đủ số lượng tồn kho.'
                ]);
            }
        }
